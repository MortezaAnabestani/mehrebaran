...
const PodiumStep = ({ entry, rank, color, height, delay }: { entry?: IEntry; rank: number; color: string; height: string; delay: number }) => {
    ...
    >
      ...
      <div className="relative mb-4">
        {rank === 1 && (
          ...
          >
            <OptimizedImage src={"/icons/crown.svg"} alt="تاج مقام اول" width={100} height={100} />
          </motion.div>
        )}
        <div
      ...
      <div className="text-center mb-2">
        <div className="font-bold text-gray-800 text-sm md:text-base truncate max-w-[100px] md:max-w-[140px]">
...
const Podium = ({ topThree }: { topThree: IEntry[] }) => {
  ...
  return (
    <div className="flex items-end justify-center gap-2 md:gap-4 h-[260px] md:h-[320px] mb-6 md:mb-8 px-2 md:px-4">
      <PodiumStep
        ...
        height="100px" // Adjusted string values for simplicity, could be dynamic
        delay={0.2}
      />
      <PodiumStep
        entry={first}
...
const LeaderboardClient: React.FC = () => {
  ...
  const category = (searchParams.get("category") || "points") as "points" | "needs_created" | "needs_supported" | "tasks_completed";
  const period = (searchParams.get("period") || "all_time") as "all_time" | "monthly" | "weekly" | "daily";

  const setCategory = (val: typeof category) => {
    const params = new URLSearchParams(searchParams.toString());
    params.set("category", val);
    router.replace(`${pathname}?${params.toString()}`, { scroll: false });
  };

  const setPeriod = (val: typeof period) => {
    const params = new URLSearchParams(searchParams.toString());
  ...
  return (
    <ProtectedRoute>
      <main className="min-h-screen bg-[#eef2f6] text-gray-800 font-sans pb-24">
              ...
              <div className="bg-gradient-to-r from-white to-blue-50/50 rounded-[16px] md:rounded-[20px] p-4 md:p-6 flex flex-col md:flex-row items-center justify-between gap-4 md:gap-6 border border-white/50">
                <div className="flex items-center gap-3 md:gap-5 w-full md:w-auto">
                  ...
                  <div className="text-center md:text-right">
                    <div className="text-base md:text-xl font-bold text-gray-800">{user?.name}</div>
