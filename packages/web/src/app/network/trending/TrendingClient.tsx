...
import type { INeed, IUser } from "common-types";
...
/**
 * Trending Client - Popular needs and users
 * Redesigned with Skeuomorphism and Semantic HTML
 */
const TrendingClientContent: React.FC = () => {
  const router = useRouter();
  const searchParams = useSearchParams();

  // State
  const activeCategory = (searchParams.get("category") as Category) || "needs";
  ...
  const fetchTrendingContent = useCallback(async (signal?: AbortSignal) => {
    try {
      setIsLoading(true);
  ...
  useEffect(() => {
    ...
    fetchTrendingContent(controller.signal);
  ...
  const handleUserFollowChange = useCallback(() => {
    fetchTrendingContent();
  }, [fetchTrendingContent]);
