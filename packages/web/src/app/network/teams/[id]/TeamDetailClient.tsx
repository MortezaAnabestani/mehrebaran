...
const TeamDetailClient: React.FC = () => {
  ...
  const fetchTeam = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      ...
      const message = axios.isAxiosError(err)
        ...
        : undefined;
      setError(message || "خطا در دریافت اطلاعات تیم");
    } finally {
      if (!signal?.aborted) {
        setIsLoading(false);
      }
  ...
  if (isLoading) {
    return (
      <ProtectedRoute>
        <div className="flex items-center justify-center min-h-screen bg-[#f4f7f9]" aria-live="polite">
          <div className="text-center p-8 bg-white rounded-2xl shadow-lg">
            <div className="animate-spin rounded-full h-12 w-12 border-4 border-slate-200 border-t-[#007acc] mx-auto mb-4" aria-hidden="true"></div>
      ...
      <ProtectedRoute>
        <div className="flex items-center justify-center min-h-screen bg-[#f4f7f9]">
          <div className="text-center p-8 bg-white rounded-2xl shadow-lg max-w-md w-full">
            <div className="w-16 h-16 bg-red-50 text-red-500 rounded-full flex items-center justify-center mx-auto mb-4 text-2xl" aria-hidden="true">
              !
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <aside className="lg:col-span-3 space-y-6">
              ...
              <SkeuoCard className="p-6 relative overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-[#007acc] to-[#005fa3]" aria-hidden="true"></div>
                ...
                <div className="mt-6 space-y-2">
                  {!isUserMember && team.status === "active" && !team.isPrivate && (
                    <SmartButton
                      variant="morange"
                      size="md"
              ...
              {stats && (
                <SkeuoCard className="p-6">
                  <h3 className="font-bold text-lg mb-4 text-slate-800 flex items-center gap-2">
                    <span className="w-1 h-5 bg-[#007acc] rounded-full" aria-hidden="true"></span>
                    آمار عملکرد
                  </h3>
                  <div className="space-y-4">
                    ...
                    <div className="pt-2">
                      ...
                      <div className="w-full bg-slate-200 rounded-full h-3 shadow-[inset_0_1px_3px_rgba(0,0,0,0.2)] overflow-hidden" role="progressbar" aria-valuenow={stats.teamProgress || 0} aria-valuemin={0} aria-valuemax={100}>
                        <div
              ...
              <SkeuoCard className="p-6">
                ...
                <div className="space-y-3 max-h-[300px] overflow-y-auto pr-1 custom-scrollbar">
                  {team.members?.slice(0, 10).map((member, index) => {
                    ...
                    return (
                      <div
                        key={userId?.toString() || `member-${index}`}
                ...
                {blockedTasks.length > 0 && (
                  <div className="mt-8 p-4 bg-red-50/50 border border-red-100 rounded-xl shadow-sm">
                    <h3 className="font-bold text-sm mb-3 text-red-600 flex items-center gap-2">
                      ...
                      تسک‌های مسدود شده ({blockedTasks.length})
                    </h3>
