...
const NeedHeader: React.FC<NeedHeaderProps> = ({ need, isFollowing, onFollow }) => {
  ...
  return (
    <header className="flex items-center justify-between px-6 py-5 bg-gradient-to-b from-gray-50 to-gray-100 border-b border-gray-200/80 shadow-[0_1px_2px_rgba(0,0,0,0.02)]">
      {/* بخش اطلاعات کاربر */}
      <div className="flex items-center gap-4">
        {/* آواتار با استایل Skeuomorphic (برجسته و قاب‌دار) */}
        <div className="relative w-14 h-14 rounded-full p-1 bg-gradient-to-br from-white to-gray-200 shadow-[2px_2px_5px_rgba(0,0,0,0.1),-2px_-2px_5px_rgba(255,255,255,0.8)]">
        ...
        <div className="flex flex-col justify-center">
          <h3 className="text-base font-bold text-gray-800 tracking-tight leading-tight mb-0.5 drop-shadow-sm">
            {getCreatorName(need)}
          </h3>
          <time dateTime={isoDate} className="text-xs font-medium text-gray-500/90">
            {formattedDate}
          </time>
        </div>
      </div>
      ...
      <button
        ...
        className={`
          relative overflow-hidden px-6 py-2.5 rounded-xl text-sm font-bold transition-all duration-200 ease-out
