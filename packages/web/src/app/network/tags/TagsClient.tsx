...
const TagCard: React.FC<{ tag: ITag }> = ({ tag }) => (
  ...
  >
    <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity text-blue-500" aria-hidden="true">
      <svg
    ...
    <div className="w-12 h-12 mb-3 rounded-full bg-blue-50 flex items-center justify-center text-[#007acc] shadow-inner" aria-hidden="true">
      <span className="text-2xl font-black">#</span>
    </div>

    <h2 className="text-lg font-bold text-gray-800 group-hover:text-[#007acc] transition-colors text-center">
      {tag.name}
    </h2>
...
const TagsClient: React.FC = () => {
  ...
  const activeTab: "popular" | "trending" = tabParam === "trending" ? "trending" : "popular";
  const [searchQuery, setSearchQuery] = useState<string>(qParam || "");
  const [searchResults, setSearchResults] = useState<ITag[]>([]);

  const setActiveTab = (tab: "popular" | "trending") => {
  ...
  const executeSearch = useCallback(async (query: string, signal?: AbortSignal) => {
    ...
    try {
      setIsLoading(true);
      const response = await socialService.searchTags(query);
      if (signal?.aborted) return;
  ...
  useEffect(() => {
    ...
    if (qParam) {
      executeSearch(qParam, controller.signal);
    }
    return () => controller.abort();
  }, [fetchTags, qParam, executeSearch]);
        ...
        <header className="relative w-full py-20 overflow-hidden bg-gradient-to-br from-[#007acc] to-[#005fa3] shadow-lg">
          ...
          <div className="relative z-10 container mx-auto px-6 md:px-12 text-center md:text-right">
            <h1 className="text-3xl md:text-5xl font-black text-white mb-4 drop-shadow-md flex items-center justify-center md:justify-start gap-3">
              <span className="text-blue-200 opacity-80" aria-hidden="true">#</span>
              کاوش تگ‌ها
            </h1>
          ...
          <section className="mb-12 space-y-8">
            <form onSubmit={handleSearch} className="relative max-w-2xl mx-auto md:mx-0">
              <div className="relative group">
                ...
                />
                <div className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-[#007acc] transition-colors" aria-hidden="true">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="h-6 w-6"
                    fill="none"
          ...
          <section aria-live="polite" className="min-h-[300px]">
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-red-100 shadow-sm">
                <div className="text-red-500 bg-red-50 p-4 rounded-full mb-4" aria-hidden="true">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="h-8 w-8"
                    fill="none"
