...
const ChevronDownIcon = () => (
  <svg aria-hidden="true" className="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
  </svg>
);

const TeamsClient: React.FC = () => {
  ...
  const activeTab = (searchParams.get("tab") as "all" | "my") || "all";
  const selectedStatus = searchParams.get("status") || "";
  const selectedFocusArea = searchParams.get("focusArea") || "";
  ...
  const setActiveTab = (tab: "all" | "my") => updateUrlParam("tab", tab);
  const setSelectedStatus = (status: string) => updateUrlParam("status", status);
  const setSelectedFocusArea = (focusArea: string) => updateUrlParam("focusArea", focusArea);
  ...
  const fetchTeams = useCallback(async (signal?: AbortSignal) => {
    try {
      ...
      if (selectedStatus) params.status = selectedStatus as ITeam["status"];
      ...
      const errMsg = err instanceof Error ? err.message : "خطا در دریافت اطلاعات تیم‌ها";
      setError(errMsg);
    } finally {
      if (!signal?.aborted) {
        setIsLoading(false);
  ...
  }, [selectedStatus, selectedFocusArea, activeTab]);
        ...
        <div className="max-w-7xl mx-auto px-6 py-8">
          ...
          <section className="bg-white rounded-2xl p-6 mb-10 shadow-[4px_4px_10px_rgba(0,0,0,0.03),-4px_-4px_10px_rgba(255,255,255,0.8)] border border-white">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Status Filter */}
              <div className="group">
                <label htmlFor="status-filter" className="block text-xs font-bold text-gray-500 mb-2 mr-1 uppercase tracking-wider">
                  وضعیت تیم
                </label>
              ...
              {/* Focus Area Filter */}
              <div className="group">
                <label htmlFor="focus-area-filter" className="block text-xs font-bold text-gray-500 mb-2 mr-1 uppercase tracking-wider">
                  حوزه فعالیت
                </label>
                <div className="relative">
                  <select
